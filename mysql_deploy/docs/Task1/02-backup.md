...
full **mysqldump** of *all databases*, compressed and uploaded to S3/MinIO, with server-side verification and simple retention.

> **Prereqs**
> - Namespace: `arvan-test`
> - Secret: `s3-auth` with `S3_ENDPOINT`, `S3_ACCESS_KEY_ID_WRITE`, `S3_SECRET_ACCESS_KEY_WRITE`, `S3_BUCKET`
> - DB root secret: `mysql-secret`

---

## 1) Apply the CronJob

If you keep your CronJob in `cronjob-backup.yaml`, simply:

```bash
kubectl -n arvan-test apply -f cronjob-backup.yaml
```


## 2) cronjob-backup.yaml
```yaml
apiVersion: batch/v1
kind: CronJob
metadata:
